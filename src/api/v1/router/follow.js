...
router.post('/:id_follower', Auth.authenGTUser, async(req, res, next) => {
    try {
        ...
        let id_following = Auth.getUserID(req);
        ...
        if (accExists) {
            ...
            const account_name_send = await Account.selectName(id_following)
            const hasToken = await Account.hasDeviceToken(id_follower)
            const token_device = hasToken ? await Account.getAccountDevice(id_follower) : null
            const message = {
                data: {
